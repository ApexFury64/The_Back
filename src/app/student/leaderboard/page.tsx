...
export default function StudentLeaderboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/student/dashboard?userEmail=${email}`)
      .then(res => res.json())
      .then(d => {
        setData(d);
  ...
  if (loading || !data || data.error) {
    return (
      <DashboardLayout role="student" userName={userName || "Student"} schoolName={schoolName || "AI Tutor"} pageTitle="Class Leaderboard" pageSubtitle="Loading...">
  ...
  const top3 = leaderboard.length >= 3
  ...
  return (
    <DashboardLayout
      role="student"
      userName={userName || data.user?.name || "Student"}
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {top3.length >= 3 && (
          <div className="flex justify-center items-end gap-2 sm:gap-6 pt-10 h-64">
            {top3.map((student: any, index: number) => {
        ...
        <div className="glass-card overflow-hidden">
          <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">Based on quiz scores</span>
          </div>
